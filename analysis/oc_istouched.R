...
# convert extant=true to has_log10_mincut to assist easily-confused readers

x[no_mincut=='FALSE', log10_mincut:='yes']
x[no_mincut=='TRUE', log10_mincut:='no']

x$log10_mincut <- factor(x$log10_mincut,levels=c('yes','no'))


p1 <- ggplot(x, aes(fill=log10_mincut, y=count, x=clustering)) + geom_bar(position="stack",stat="identity") + 
theme_bw() 
...
colnames(long_df)[2] <- 'log10_mincut'
long_df$clustering <- factor(long_df$clustering,levels=c('leiden 0.5','leiden 0.1','leiden 0.01','leiden 0.001'))
long_df$log10_mincut <- factor(long_df$log10_mincut, levels=c('yes','no'))
p3 <- ggplot(long_df,aes(x=clustering,fill=log10_mincut,y=value)) + geom_bar(position="stack",stat="identity") + 
theme_bw() 
