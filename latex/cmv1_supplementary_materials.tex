...
\caption[CM pipeline on the empirical physics citation network and its model LFR graph for r=0.01]{\textbf{CM pipeline on the empirical physics citation network and its model LFR graph for r=0.01.} The left panel shows the community size distribution in each step of running CM and the right panel shows a comparison between the community size distribution of the original Leiden clustering, the LFR ground-truth communities and the Leiden clustering of the LFR network.}
...
\caption[IKC Clusters of the CEN Impacted by CM]{\textbf{IKC Clusters of the Curated Exosome Network (CEN) Impacted by CM}. For IKC followed by CM processing, each bar shows the total number of clusters of size at least 11. The fraction of the clusters that are affected by the CM processing is colored in red and the fraction in blue are for those that are not affected by the CM processing (because their min cut size is at least that large). Overall, 1.41\% of the input IKC clusters were modified by CM.}
