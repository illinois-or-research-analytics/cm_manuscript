...
objective is a partitioning of the vertices into disjoint subsets, so that each of the subsets defines a community. While variants of this formulation exist [cite Coscia], constructing 
overlapping clusters for example, this is a common approach to community detection that has been used in many studies [cites]. Community detection in large networks has 
broad applications that include, in scientometrics, the detection of research areas, and author communities [cite].
...
into a staging view. The script was designed to remove duplicate records, self-citations, and parallel edges. DOIs were also checked for case differences. and no duplicate DOIs differing in case only were found. 
